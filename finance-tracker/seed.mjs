...
async function seedTransactions() {
    ...
    for (let year = new Date().getFullYear(); year > new Date().getFullYear() - 2; year--) {
        for (let i = 0; i < 10; i++) {
            ...
            if (typeBias < 0.85) {
                type = 'Expense'
                category = faker.helpers.arrayElement(expenseCategories) // Category only for 'Expense'
            } else if (typeBias < 0.95) {
                type = 'Income'
                category = faker.helpers.arrayElement(incomeCategories) // Category only for 'Expense'
            } else {
